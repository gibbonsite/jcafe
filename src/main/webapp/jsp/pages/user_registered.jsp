...
<div class="page">
    ...
    <div class="container justify-content-center col-12 col-sm-6 mt-3" >
        <fmt:message key="registration.user_registered"/>
        <c:choose>
	        <c:when test="${empty param.next_page}">
		        <fmt:message key="registration.to_sign_in"/> <a href="${absolutePath}/jsp/pages/sign_in.jsp"><fmt:message key="header.sign_in"/></a>.
	        </c:when>
	        <c:otherwise>
       	        <fmt:message key="admin.continue_work_by_next_page"/> <a href="${absolutePath}${param.next_page}"><fmt:message key="admin.next_page"/></a>.
	        </c:otherwise>
        </c:choose>
    </div>
