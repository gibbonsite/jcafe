...
<div class="page">
    ...
    <div class="container justify-content-center col-12 col-sm-6 mt-3" >
        <c:choose>
	        <c:when test="${param.role == 'CLIENT' }">
		        <fmt:message key="order.order_not_cancelled"/>
		        <fmt:message key="order.return_to_orders"/> 
		        <a href="${absolutePath}/controller?command=go_to_orders_page"><fmt:message key="header.orders"/></a>.
	        </c:when>
	        <c:otherwise>
		        <fmt:message key="order.order_state_not_changed"/>
		        <fmt:message key="order.return_to_orders"/> 
		        <a href="${absolutePath}/controller?command=find_all_orders"><fmt:message key="header.all_orders"/></a>.
	        </c:otherwise>
        </c:choose>
